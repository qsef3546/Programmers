...
def solution(s, skip, index):
    alpha_skip = sorted(set(ascii_lowercase) - set(skip)) # 전체 알파벳 - skip할 알파벳 리스트
    answer = list(map(lambda x: alpha_skip[(alpha_skip.index(x)+index) % len(alpha_skip)], list(s))) # 각 알파벳 + index 만큼 이동 'z' 다음 이동시 처음으로 돌아가야함
    return answer
