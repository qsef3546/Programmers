...
def solution(str1, str2):
    str1 = str1.casefold() #문자만 소문자로 변환
    str2 = str2.casefold()
    st1 = list(filter(lambda x: x.isalpha(), [str1[x:x + 2] for x in range(len(str1) - 1)])) #영어로만 이루어지는 두 글자만 리스트화
    st2 = list(filter(lambda x: x.isalpha(), [str2[x:x + 2] for x in range(len(str2) - 1)]))
    ...
    return int(len(intersact) / len(union) * 65536) if union else 65536 # 합집합이 0 이면 교집합도 있을 수 없다.
