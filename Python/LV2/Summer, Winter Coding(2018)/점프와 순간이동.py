...
# 단, 건전지 사용량을 줄이기 위해 점프로 이동하는 것은 최소로 하려고 합니다. 아이언 슈트 구매자가 이동하려는 거리 N이 주어졌을 때, 사용해야 하는 건전지 사용량의 최솟값을
...
def solution(n):
    ans = 1 # N = 2ⁿ 이거나 N = 1일경우 1칸만 움직이면 된다.
    p = 0
    while n > 2 ** p: p += 1 # 2ⁿ < N 을 만족하는 N의 최댓값 구하기 현재 코드에선 n-1 해줘야 최댓값 구할 수 있다.
    if n < 2 ** p:
        ans += list(bin(n - 2 ** (p - 1))[2:]).count('1')  # N - 2^p 를 2진법으로 변환 하면, 1의 개수가 점프해야할 거리(건전지 사용량) 이다.
    return ans
