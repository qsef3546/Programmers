 ...
 *    정수 n, m과 다시 페인트를 칠하기로 정한 구역들의 번호가 담긴 정수 배열 section이 매개변수로 주어질 때 롤러로 페인트칠해야 하는 최소 횟수를
...
int solution(int n, int m, vector<int> section) {
    int answer = 0;
    int idx = 0; //칠해야하는 시작 위치
    for (auto it : section) {
        if (it < idx) { //칠해야하는 it 위치가 idx 위치보다 낮은 번호라면 it는 이미 칠한 위치이므로 다음 번호로 이동
            it++;
            continue;
        }
        idx = it+m; //칠해야하는 위치부터 m 만큼 칠하고 그 다음위치 idx 에 대입
        answer++;
    }
    return answer;
}
...
void main() {
    ...
    int answer = solution(n, m, solution);
}