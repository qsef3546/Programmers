 ...
 * 예를 들어, 15번으로 지정된 기사단원은 15의 약수가 1, 3, 5, 15로 4개 이므로, 공격력이 4인 무기를 구매합니다. 만약, 이웃나라와의 협약으로
 * 정해진 공격력의 제한수치가 3이고 제한수치를 초과한 기사가 사용할 무기의 공격력이 2라면, 15번으로 지정된 기사단원은 무기점에서 공격력이 2인 무기를 구매합니다.
...
int solution(int number, int limit, int power) {
    int answer = 0;
    vector<int> knight; //약수 개수(=공격력) 저장하는 vector
    for (int i = 1; i <= number; i++) knight.push_back(yak(i)); //약수 개수(=공격력) 저장
    for (auto it : knight) answer += it > limit ? power : it; //공격력이 limit 보다 클경우 limit을, 작을경우 해당 공격력을 answer에 누적 대입
    return answer;
}
